...
export async function registerProvider(
...
): Promise<GLMChatProvider> {
  const provider = new GLMChatProvider(context);

  context.subscriptions.push(
    vscode.commands.registerCommand("glm-copilot.setApiKey", () =>
      provider.configureApiKey(),
    ),
    vscode.commands.registerCommand("glm-copilot.queryUsage", () =>
      provider.queryUsage(),
    ),
    vscode.commands.registerCommand("glm-copilot.clearApiKey", () =>
      provider.clearApiKey(),
    ),
    vscode.commands.registerCommand("glm-copilot.setVisionModel", () =>
  ...
  try {
    ...
    logger.warn(
      "Model picker refresh failed; will retry on next chat interaction",
      error,
    );
  }
...
async function activateCopilotChat(): Promise<void> {
  try {
    ...
    logger.warn(
      "Copilot Chat activation unavailable; model picker refresh may be delayed",
      error,
    );
  }
}
