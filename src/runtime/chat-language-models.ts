...
export async function seedChatLanguageModelDefaults(
  ...
  const migratedVersion = context.globalState.get<number>(
  ...
  if (!filePath) {
    logger.warn(
      "Unable to locate chatLanguageModels.json for GLM defaults migration",
    );
  ...
  try {
    ...
    logger.warn(
      `Failed to seed GLM chat model defaults: file=${filePath}`,
      error,
    );
  }
}
...
async function upsertGLMDefaults(filePath: string): Promise<boolean> {
  ...
  for (const model of MODELS) {
    ...
    if (
      setting.reasoningEffort === undefined ||
...
async function readExistingChatLanguageModels(
  filePath: string,
...
function getOrCreateGLMGroup(
  groups: ChatLanguageModelGroup[],
): ChatLanguageModelGroup {
