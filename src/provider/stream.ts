...
export function streamChatCompletion({
  ...
  const cancelListener = observeCancellationToken(
    token,
    ...
    .streamChatCompletion(
      prepared.request,
      {
        onContent: (content: string) => {
          ...
          reportInitialResponseNoticeOnce(
            progress,
            state,
            initialResponseNotice,
          );
...
function reportReplayMarker(
  ...
  try {
    ...
    logger.warn(
      formatRequestLogLine(
        prepared.requestKind,
        "Failed to report replay marker",
      ),
      error,
    );
  }
...
function reportCopilotContextUsage(
  ...
  const cachedTokens = Math.max(
    0,
    Math.min(rawCached, Math.max(usage.prompt_tokens, 0)),
  );
  const data = {
...
function reportUsageCost(
  ...
  try {
    ...
    logger.warn(
      formatRequestLogLine(requestKind, "Failed to update usage status"),
      error,
    );
  }
}
