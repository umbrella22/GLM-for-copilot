...
function buildPrepared({
  ...
  (prepared as unknown as { __diagnostics: typeof diagnostics }).__diagnostics =
    diagnostics;
  return prepared;
}
...
function getDiagnostics(prepared: PreparedChatRequest) {
  return (
    prepared as unknown as { __diagnostics: ReturnType<typeof fakeDiagnostics> }
  ).__diagnostics;
}
