...
describe("endpoint helpers", () => {
  it("resolves apiMode and region endpoint presets", () => {
    expect(resolvePresetBaseUrl("coding-plan", "china")).toBe(
      GLM_CN_CODING_BASE_URL,
    );
    expect(resolvePresetBaseUrl("standard", "china")).toBe(
      GLM_CN_GENERAL_BASE_URL,
    );
  ...
  it("resolves API key pages from apiMode and region", () => {
    expect(resolveApiKeyUrl("coding-plan", "china")).toBe(
      GLM_CN_CODING_API_KEY_URL,
    );
    expect(resolveApiKeyUrl("standard", "china")).toBe(
      GLM_CN_GENERAL_API_KEY_URL,
    );
  ...
  it("identifies official GLM platforms by host", () => {
    expect(
      identifyOfficialGLMPlatform(
        `https://${GLM_INTERNATIONAL_API_HOST}/api/paas/v4`,
      ),
    ).toBe("zai");
    expect(
      identifyOfficialGLMPlatform(`https://${GLM_CN_API_HOST}/api/paas/v4`),
    ).toBe("zhipu");
    expect(
      identifyOfficialGLMPlatform(
        `https://${GLM_CN_LEGACY_API_HOST}/api/paas/v4`,
      ),
    ).toBe("zhipu");
  });
  ...
  it("does not classify custom or invalid URLs as official", () => {
    expect(
      identifyOfficialGLMPlatform("https://proxy.example.com/v1"),
    ).toBeUndefined();
  ...
  it("resolves the Anthropic endpoint for both regions (regression: international was hardcoded to CN)", () => {
    ...
    expect(resolveAnthropicBaseUrl("international")).toBe(
      GLM_INTERNATIONAL_ANTHROPIC_BASE_URL,
    );
  });
});
...
describe("endpoint preset resolver", () => {
  it("resolves every preset to its official base URL", () => {
    ...
    expect(resolveEndpointBaseUrl("china-standard")).toBe(
      GLM_CN_GENERAL_BASE_URL,
    );
    expect(resolveEndpointBaseUrl("china-anthropic")).toBe(
      GLM_CN_ANTHROPIC_BASE_URL,
    );
    expect(resolveEndpointBaseUrl("international-coding")).toBe(
      GLM_INTERNATIONAL_CODING_BASE_URL,
    );
  ...
  it("resolves every preset to its API key management page", () => {
    expect(resolveEndpointApiKeyUrl("china-coding")).toBe(
      GLM_CN_CODING_API_KEY_URL,
    );
    expect(resolveEndpointApiKeyUrl("china-standard")).toBe(
      GLM_CN_GENERAL_API_KEY_URL,
    );
    expect(resolveEndpointApiKeyUrl("china-anthropic")).toBe(
      GLM_CN_CODING_API_KEY_URL,
    );
  ...
  it("maps each preset to its implied wire protocol", () => {
    ...
    expect(resolveEndpointProtocol("international-anthropic")).toBe(
      "anthropic",
    );
  });
});
...
describe("legacy endpoint derivation", () => {
  it("maps the default tuple to china-coding", () => {
    expect(deriveEndpointPreset("china", "coding-plan", "openai")).toBe(
      "china-coding",
    );
  });

  it("maps standard mode on both regions", () => {
    expect(deriveEndpointPreset("china", "standard", "openai")).toBe(
      "china-standard",
    );
  ...
  it("lets apiProtocol=anthropic override apiMode and picks the right region (regression)", () => {
    ...
    expect(deriveEndpointPreset("china", "coding-plan", "anthropic")).toBe(
      "china-anthropic",
    );
    expect(deriveEndpointPreset("china", "standard", "anthropic")).toBe(
      "china-anthropic",
    );
    expect(
      deriveEndpointPreset("international", "coding-plan", "anthropic"),
