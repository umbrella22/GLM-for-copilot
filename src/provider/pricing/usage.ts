...
export function estimateUsageCost(
  ...
  const inputCost =
    (cacheHitInputTokens / TOKENS_PER_MILLION) *
      effectivePricing.cacheHitInput +
    (cacheMissInputTokens / TOKENS_PER_MILLION) *
      effectivePricing.cacheMissInput;
  const outputTokens = usage.completion_tokens;
  const outputCost =
    (outputTokens / TOKENS_PER_MILLION) * effectivePricing.output;
  ...
  return {
    ...
    pricing: {
      ...
      ...("label" in effectivePricing
        ? { tierLabel: effectivePricing.label }
        : {}),
    },
  };
}
...
function selectPricingTier(
  ...
  return (
    pricing.tiers?.find((tier) => {
      const aboveMin =
        tier.minPromptTokens === undefined ||
        promptTokens >= tier.minPromptTokens;
      const belowMax =
        tier.maxPromptTokens === undefined ||
        promptTokens < tier.maxPromptTokens;
