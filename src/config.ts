...
export function getApiMode(): ApiMode {
  const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
  return (
    normalizeApiMode(config.get<string>("apiMode"), DEFAULT_API_MODE) ??
    DEFAULT_API_MODE
  );
}
...
export function getRegion(): ApiRegion {
  const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
  return (
    normalizeApiRegion(config.get<string>("region"), DEFAULT_API_REGION) ??
    DEFAULT_API_REGION
  );
}
...
export function getApiProtocol(): ApiProtocol {
  ...
  const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
  const explicitEndpoint = normalizeEndpointPreset(
    config.get<string>("endpoint"),
  );
  if (explicitEndpoint) {
    return protocol;
  }
  const legacyProtocol = normalizeApiProtocol(
    config.get<string>("apiProtocol"),
    protocol,
  );
...
export async function migrateLegacyDebugSetting(): Promise<void> {
  ...
  if (
    ...
    await migrateLegacyDebugSettingAtScope(
...
export async function migrateLegacyEndpointSettings(): Promise<void> {
  ...
  const effectiveEndpoint = normalizeEndpointPreset(
    config.get<string>("endpoint"),
  );
...
function getConfiguredDebugMode(
  config: vscode.WorkspaceConfiguration,
): DebugMode | undefined {
...
function normalizeCustomModel(entry: unknown): ModelDefinition | undefined {
  ...
  return {
    ...
    maxInputTokens: getPositiveInteger(
