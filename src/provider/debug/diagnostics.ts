...
import type {
  VisionResolutionStats as VisionPipelineStats,
  VisionProxySource,
} from "../vision";
...
export function logToolFlowDiagnostics({
  ...
  const activateToolCount =
    tools?.reduce(
      (count, tool) =>
        count + (tool.name.startsWith(ACTIVATE_TOOL_PREFIX) ? 1 : 0),
      0,
    ) ?? 0;
...
class DefaultCacheDiagnosticsRecorder implements CacheDiagnosticsRecorder {
  ...
  private readonly lastCacheTracesByScope = new Map<
    string,
    CacheTraceSnapshot
  >();
  ...
  beginRequest(options: BeginCacheDiagnosticsOptions): CacheDiagnosticsRun {
    ...
    const previousCacheTrace = this.previousCacheTraces.get(
      getCacheTraceStoreKey(cacheTrace),
    );
    ...
    const cacheTraceComparison = compareCacheTraceSnapshots(
      previousCacheTrace,
      cacheTrace,
    );
    ...
    const traceKeyChangeComparison =
      ...
      immediateTraceKeyChanged &&
      sameImmediateComparisonScope
    ...
    if (visionTrace) {
      logger.info(
        formatRequestLogLine(
          requestKind,
          `[cache-trace #${requestId}] ${visionTrace}`,
        ),
      );
    }
    if (cacheTraceComparison) {
      ...
      for (const warning of getCacheTraceComparisonWarnings(
        cacheTraceComparison,
      )) {
        logger.warn(
          formatRequestLogLine(
            requestKind,
            `[cache-trace #${requestId}] ${warning}`,
          ),
        );
      }
    }
    ...
    for (const infoLine of getCacheTraceInfoLines(cacheTrace)) {
      logger.info(
        formatRequestLogLine(
          requestKind,
          `[cache-trace #${requestId}] ${infoLine}`,
        ),
      );
    }
...
function formatSegmentTrace(segment: ConversationSegment): string {
  ...
  if (segment.reason === "markerFound") {
    ...
    const markerLocation =
      segment.markerMessageIndex === undefined ||
      segment.markerPartIndex === undefined
        ? ""
    ...
    const markerLocation =
      ...
      segment.markerPartIndex === undefined
        ? ""
        : ` at=message#${segment.markerMessageIndex}:part#${segment.markerPartIndex}`;
    const markerError = segment.markerError
      ? ` error=${segment.markerError}`
      : "";
...
function formatReplayMarkerReport(info: ReplayMarkerReportInfo): string {
  ...
  const markerBytes =
    info.markerBytes === undefined ? "" : ` markerBytes=${info.markerBytes}`;
  const visionTextChars =
    info.visionTextChars === undefined
      ? ""
      : ` visionTextChars=${info.visionTextChars}`;
  const reasoningTextChars =
    info.reasoningTextChars === undefined
      ? ""
      : ` reasoningTextChars=${info.reasoningTextChars}`;
  const segmentOnly =
    info.segmentOnly === undefined
      ? ""
      : ` segmentOnly=${info.segmentOnly ? "true" : "false"}`;
...
function summarizeHostPromptTrace(
  ...
  for (const [index, message] of messages.entries()) {
    ...
    if (message.role === vscode.LanguageModelChatMessageRole.User) {
      ...
      latestUserHasCustomizationsUpdate = text.includes(
        "<customizationsUpdate>",
      );
    }
  }
  ...
  return {
    ...
    systemRole: systemMessage
      ? formatVscodeMessageRole(systemMessage.role)
      : null,
...
function shouldLogHostPromptTrace(trace: HostPromptTrace): boolean {
  return (
    trace.customizationsUpdateCount > 0 ||
    trace.latestUserHasCustomizationsUpdate
  );
}
...
function appendNumberIfNonZero(
  parts: string[],
  name: string,
  value: number,
): void {
...
function getCacheHitRate(usage: GLMUsage): string {
  ...
  return cacheTotal > 0
    ? `${((cacheHit / cacheTotal) * 100).toFixed(0)}%`
    : "n/a";
}
...
function getCacheMissTokens(usage: GLMUsage): number {
  return (
    ...
    Math.max(usage.prompt_tokens - getCacheHitTokens(usage), 0)
  );
}
...
function summarizeVisionResolution(
  ...
  for (const [index, message] of inputMessages.entries()) {
    ...
    if (imageParts > 0) {
      ...
      const resolvedImageParts = resolvedMessage
        ? countImageDataParts(resolvedMessage)
        : 0;
      const resolvedText = resolvedMessage
        ? getMessageText(resolvedMessage)
        : "";
...
function formatVisionTrace(
  ...
  const note =
    stats.inputImageParts === 0 && stats.historyDescriptionMessages > 0
      ? " note=history-only"
      : "";
  ...
  if (pipelineStats && hasVisionPipelineActivity(pipelineStats)) {
    ...
    appendNumberIfNonZero(
      parts,
      "unavailable",
      pipelineStats.unavailableImageMessages,
    );
    appendNumberIfNonZero(parts, "failed", pipelineStats.failedImageMessages);
    appendNumberIfNonZero(
      parts,
      "markerChars",
      pipelineStats.markerVisionTextChars,
    );
    appendNumberIfNonZero(
      parts,
      "invalidMarkerVision",
      pipelineStats.invalidMarkerVisionMetadata,
    );
  } else {
  ...
  appendNumberIfNonZero(
    parts,
    "historyDescriptions",
    stats.historyDescriptionMessages,
  );
  return parts.join(" ") + note;
}

function hasVisionPipelineActivity(
  stats: VisionPipelineStats | undefined,
): boolean {
  if (!stats) {
    return false;
  }
...
function formatVscodeMessageTrace(
  ...
  const traces = messages.map((msg, index) => {
    ...
    for (const part of msg.content) {
      if (part instanceof vscode.LanguageModelTextPart) {
        ...
        if (part.mimeType.startsWith("image/")) {
          ...
          dataPartMimes.set(
            part.mimeType,
            (dataPartMimes.get(part.mimeType) ?? 0) + 1,
          );
        }
    ...
    if (thinkingParts) {
      parts.push(
        `thinking=${thinkingParts}:chars=${thinkingChars}:types=${[
          ...thinkingValueTypes,
        ].join("+")}:hashes=${thinkingHashes.join(",")}`,
      );
    }
...
function formatVscodeMessageRole(
...
function isLanguageModelThinkingPart(
  part: unknown,
...
export function createCacheTraceSnapshot(
  ...
  return {
    ...
    requestKind,
    model: request.model,
    cacheTraceKey: hashString(
      `${request.model}:${firstMessage?.hash ?? "empty"}`,
    ),
  ...
  return {
    ...
    commonPrefixSummaryPercent:
      current.redactedComparisonInput.length > 0
        ? (commonPrefixSummaryChars / current.redactedComparisonInput.length) *
          100
        : 100,
...
export function formatCacheTraceSnapshot(snapshot: CacheTraceSnapshot): string {
  ...
  appendNumberIfNonZero(
    parts,
    "imageDescriptions",
    stats.imageDescriptionMessages,
  );
  appendNumberIfNonZero(parts, "largeMessages", stats.largeMessages);
  return parts.join(" ");
}

export function formatCacheTraceDetailLines(
...
export function formatCacheTraceComparison(
...
export function formatCacheTraceComparisonDetailLines(
  comparison: CacheTraceComparison,
): string[] {
  if (
    comparison.firstChangedMessageIndex === undefined ||
...
export function getCacheTraceWarnings(snapshot: CacheTraceSnapshot): string[] {
  ...
  const recoveryLostEmptyMessages = countEmptyReasoningFallbackMessages(
    snapshot,
    "recovery-lost",
  );
  ...
  const unknownEmptyMessages = countEmptyReasoningFallbackMessages(
    snapshot,
    "unknown",
  );
...
type EmptyReasoningFallbackCause =
  | "upstream-no-reasoning"
  | "recovery-lost"
  | "unknown";
...
function isReasoningRequiredEmptyFallback(
  message: CacheTraceMessageSummary,
): boolean {
  return (
    message.emptyReasoning &&
    (message.toolCalls > 0 || message.afterToolResultKind === "final")
  );
}
...
export function getCacheTraceComparisonWarnings(
  comparison: CacheTraceComparison,
): string[] {
  const warnings: string[] = [];
  if (
...
function formatSystemPromptChange(
  change: CacheTraceSystemPromptChange,
): string {
...
function formatContentSectionSummary(
  summary: CacheTraceContentSectionSummary | undefined,
): string {
  if (!summary) {
    return "missing";
  }
...
function summarizeMessage(
  ...
  const toolCallArgumentChars =
    message.tool_calls?.reduce(
      (sum, toolCall) => sum + toolCall.function.arguments.length,
      0,
    ) ?? 0;
  const reasoningChars = message.reasoning_content?.length ?? 0;
  const toolCalls = message.tool_calls?.length ?? 0;
  const assistantAfterToolResult =
    message.role === "assistant" && followsToolResult;
  ...
  const imageDescriptionCount = countLiteral(
    message.content,
    "[Image Description:",
  );
  const unableImageCount = countLiteral(
    message.content,
  ...
  return {
    ...
    missingToolReasoning:
      message.role === "assistant" && toolCalls > 0 && !hasReasoningContent,
    followsToolResult: assistantAfterToolResult,
    afterToolResultKind,
    missingPostToolReasoning: assistantAfterToolResult && !hasReasoningContent,
    missingPostToolCallReasoning:
      afterToolResultKind === "tool-call" && !hasReasoningContent,
    missingPostToolFinalReasoning:
      afterToolResultKind === "final" && !hasReasoningContent,
    contentSections:
      index === 0 ? summarizeSystemPromptSections(message.content) : undefined,
  };
}
...
function summarizeSystemPromptSections(
  content: string,
...
function getContentLineRanges(content: string): ContentLineRange[] {
  ...
  for (let index = 0; index < content.length; index += 1) {
    ...
    const textEnd =
      index > lineStart && content.charAt(index - 1) === "\r"
        ? index - 1
        : index;
    lines.push({
...
function summarizeTools(tools: GLMTool[]): CacheTraceToolSummary[] {
  return tools.map((tool, index) => ({
    ...
    parametersHash: hashString(
      stableStringify(tool.function.parameters ?? null),
    ),
  }));
}
...
function summarizeStats(
  ...
  for (const message of messages) {
    ...
    const imageDescriptions = countLiteral(
      message.content,
      "[Image Description:",
    );
...
function formatMessageSummary(
  summary: CacheTraceMessageSummary | undefined,
): string {
  if (!summary) {
    return "missing";
  }
...
function formatRelevantMarkerSummary(
  summary: CacheTraceMessageSummary,
): string {
