...
export function setErrorActionUrl(
...
export async function createHttpError(
  ...
  return new GLMRequestError({
    ...
    diagnosticMessage: joinDiagnosticParts(
      ...
      serverMessage
        ? `serverMessage=${safeStringify(serverMessage)}`
        : undefined,
  ...
  const message =
    error instanceof GLMRequestError
      ? formatMarkdownMessage(
          error.userSummary,
          getErrorActions(error, errorActionUrlStore.get()),
        )
      : error.message;
...
function getUniversalHttpErrorActions(
  ...
  return status === 401 && url
    ? [{ labelKey: "error.action.setApiKey", url }]
    : [];
}
...
function getProviderHttpErrorLink(
  ...
  return providerId && statusKey
    ? API_PROVIDER_HTTP_ERROR_LINKS[statusKey][providerId]
    : undefined;
}
...
function getCreateApiKeyUrl(
  ...
  return status === 401
    ? getProviderHttpErrorLink(status, baseUrl)?.url
    : undefined;
}
...
function getRequestDiagnosticMessage(context: RequestErrorContext): string {
  ...
  return joinDiagnosticParts(
    ...
    request.thinking?.type
      ? `thinking=${safeStringify(request.thinking.type)}`
      : undefined,
    request.reasoning_effort
      ? `reasoningEffort=${safeStringify(request.reasoning_effort)}`
      : undefined,
    request.tool_choice
      ? `toolChoice=${safeStringify(request.tool_choice)}`
      : undefined,
...
function getHttpErrorLinkStatusKey(
  status: number,
): HttpErrorLinkStatusKey | undefined {
