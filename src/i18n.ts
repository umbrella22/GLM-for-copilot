...
const zh: Translations = {
	...
	'vision.mcp.fallbackToProxy': // [FORK] PR #15 F2
		'当前会话未检测到可用的图片 MCP 工具，已自动回退到视觉代理模式处理本请求的图片。如需使用 MCP 图片模式，请在设置中启用一个支持图片的 MCP 工具（如官方 zai-mcp-server 的 analyze_image），或通过 “glm-copilot.mcp.imageCapableTools” 添加自定义视觉工具名。\n\n',
	'vision.mcp.conflict.noImageTool': // [FORK] PR #15 F2
		'当前模型的图片处理方式为 “MCP 工具”，但本会话没有可用的图片 MCP 工具，且未配置视觉代理。MCP 图片模式需要一个能读取本地图片的 MCP 工具。请：（1）启用一个支持图片的 MCP 工具（如官方 zai-mcp-server 的 analyze_image）；或（2）配置视觉代理（在 “GLM: 管理模型与连接” 中设置视觉代理模型）；或（3）若你的模型支持原生图片，将图片处理方式改为 “原生图片”。',
};
...
const en: Translations = {
	...
	'vision.mcp.fallbackToProxy': // [FORK] PR #15 F2
		'No image-capable MCP tool is available in this session; this request\'s images are being handled by the vision proxy instead. To use MCP image mode, enable an image-capable MCP tool (e.g. the official zai-mcp-server\'s analyze_image), or add your custom vision tool name via "glm-copilot.mcp.imageCapableTools".\n\n',
	'vision.mcp.conflict.noImageTool': // [FORK] PR #15 F2
		'This model\'s vision mode is "MCP tool", but no image-capable MCP tool is available in this session and no vision proxy is configured. MCP image mode needs an MCP tool that can read local image files. Either (1) enable an image-capable MCP tool (e.g. the official zai-mcp-server\'s analyze_image); (2) configure a vision proxy (set a vision proxy model under "GLM: Manage Models and Connections"); or (3) if your model supports native images, switch its vision mode to "Native image".',
};
