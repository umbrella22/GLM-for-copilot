...
function parseReplayMarkerPart(
  part: unknown,
): ReplayMarkerParseResult | undefined {
...
export function hasReplayMarkerMetadata(
...
export function createReplayMarkerPart(
  payload: ReplayMarkerPayload,
): vscode.LanguageModelDataPart {
  const encoded = encodeReplayMarkerJson({
    segmentId: payload.segmentId,
    ...createVisionMarkerPayload(payload.visionText),
    ...createReasoningMarkerPayload(payload.reasoningText),
  });
...
export function parseReplayMarkerData(
  data: Uint8Array,
): ReplayMarkerParseResult {
  ...
  try {
    ...
    return {
      ...
      ...reasoning,
      segmentOnly: Boolean(
        segmentId.value && !vision.visionText && !reasoning.reasoningText,
      ),
      payloadFormat: decodedPayload.format,
    };
  } catch {
...
function createReasoningMarkerPayload(
