  ...
  try {
    ...
    if (message.type === "saveConfig") {
      ...
      if (payload.source === "auto") {
        ...
        await saveVSCodeVisionModelKey(
          getRequiredString(
            payload.lmModelKey,
            t("vision.panel.source.vscodeLm"),
          ),
        );
    ...
    if (message.type === "testConnection") {
      ...
      if (payload.source === "auto") {
        postStatus(
          panel,
          t("vision.panel.status.autoSelected"),
          "info",
          undefined,
          {
            kind: "test",
      ...
      if (payload.source === "vscode-lm") {
        postStatus(
          panel,
          t("vision.panel.status.vscodeLmNoHttpTest"),
          "info",
          undefined,
          {
            kind: "test",
      ...
      if (result.ok) {
        postStatus(
          panel,
          t("vision.panel.status.testSucceeded"),
          "success",
          undefined,
          {
            kind: "test",
        ...
        postStatus(
          panel,
          getVisionProxyTestFailure(result),
          "error",
          createShowLogsAction(),
          {
            kind: "test",
            testId: payload.testId,
          },
    ...
    postStatus(
      ...
      isTestError
        ? { kind: "test", testId: getWebviewTestId(message.value) }
        : undefined,
    );
  }
}
...
async function renderPanel(
  ...
  panel.webview.html = getVisionProxyPanelHtml(
    panel.webview,
...
function getConfigForPanel(
  store: VisionProxyConfigStore,
): VisionProxyConfig | undefined {
  try {
...
function getVisionProxyTestResultView(
  result: VisionProxyTestResult,
): StatusMetadata["testResult"] {
...
function getWebviewPayload(value: unknown): WebviewPayload {
  ...
  const lmModelKey =
    typeof payload.lmModelKey === "string" ? payload.lmModelKey.trim() : "";
  return {
    source,
    config,
