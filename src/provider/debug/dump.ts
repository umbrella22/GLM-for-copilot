...
export function dumpProviderInput(options: DumpProviderInputOptions): void {
  ...
  enqueueDumpWrite(
    formatRequestLogLine(requestKind, "providerInputDump"),
    async () => {
      await mkdir(context.root, { recursive: true });
      await writeJsonFile(
        paths.providerInput,
        createProviderInputSnapshot(options, context),
      );
...
export function dumpGLMRequest(
  ...
  enqueueDumpWrite(
    formatRequestLogLine(requestKind, "requestDump"),
    async () => {
      await mkdir(context.root, { recursive: true });
      await writeJsonFile(
        paths.input,
      ...
      await writeDumpObservation(
        ...
        createDumpObservation({
          ...
          model: {
            ...
            apiModelId:
              request.model === options.vscodeModelId
                ? undefined
                : request.model,
          },
          requestKind,
...
export async function ensureRequestDumpRoot(
  globalStorageUri: vscode.Uri,
): Promise<vscode.Uri> {
...
function createProviderInputDumpPaths(
  context: DumpContext,
): ProviderInputDumpPaths {
  return {
...
function createRequestDumpPaths(
  context: DumpContext,
  hasMsg0: boolean,
): RequestDumpPaths {
  return {
    ...
    msg0: hasMsg0
      ? join(context.root, `${context.basename}.msg0.txt`)
      : undefined,
  };
}
...
function createPipelineSnapshot(
  ...
  return createDumpSnapshot({
    ...
    model: {
      vscodeModelId: options.vscodeModelId,
      apiModelId:
        request.model === options.vscodeModelId ? undefined : request.model,
...
function serializeMessage(
  ...
  return {
    ...
    contentTextChars: contentParts.reduce(
      (sum, part) => sum + getContentPartTextChars(part),
      0,
    ),
    contentDataBytes: contentParts.reduce(
      (sum, part) => sum + getContentPartDataBytes(part),
      0,
    ),
    contentParts,
  };
...
function serializeContentPart(
  ...
  if (part instanceof vscode.LanguageModelToolResultPart) {
    return {
      ...
      contentParts: part.content.map((item, itemIndex) =>
        serializeContentPart(item, itemIndex),
      ),
    };
  }
...
function summarizeReplayMarker(
  marker: ReturnType<typeof parseReplayMarkerData>,
): {
  valid: boolean;
  segmentId?: string;
  payloadFormat?: string;
  ...
  return {
    ...
    visionTextHash: marker.visionText
      ? hashString(marker.visionText)
      : undefined,
    visionTextIgnoredReason: marker.visionTextIgnoredReason,
    reasoningTextChars: marker.reasoningText?.length,
    reasoningTextHash: marker.reasoningText
      ? hashString(marker.reasoningText)
      : undefined,
...
function summarizeGLMSystemPrompt(
...
function createSystemPromptSummary(
  ...
  return {
    ...
    headHash:
      messageIndex === null
        ? null
        : hashString(text.slice(0, HASH_WINDOW_CHARS)),
    tailHash:
      messageIndex === null ? null : hashString(text.slice(-HASH_WINDOW_CHARS)),
...
function summarizeVscodeCustomizations(
  ...
  for (const [index, message] of messages.entries()) {
    ...
    customizationsUpdateCountInHistory += countLiteral(
      text,
...
function summarizeGLMCustomizations(
  messages: readonly GLMMessage[],
): CustomizationsSummary {
  let customizationsUpdateCountInHistory = 0;
  let latestUserMessageIndex: number | null = null;
...
function summarizeHostSettings(): HostSettingsSummary {
  return {
    ...
    chatPlanAgentDefaultModel: getStringSetting(
      "chat",
      "planAgent.defaultModel",
    ),
    chatExploreAgentDefaultModel: getStringSetting(
      "chat",
      "exploreAgent.defaultModel",
    ),
    copilotAskAgentModel: getStringSetting(
      "github.copilot.chat",
      "askAgent.model",
    ),
    copilotImplementAgentModel: getStringSetting(
      "github.copilot.chat",
      "implementAgent.model",
    ),
    copilotExploreAgentModel: getStringSetting(
      "github.copilot.chat",
      "exploreAgent.model",
    ),
  };
...
function summarizeRequestOptions(
  ...
  const modelConfiguration = sanitizeJsonValue(
    providerOptions.modelConfiguration,
  );
...
function getContentPartTextChars(part: SerializedContentPart): number {
  ...
  if (part.type === "toolResult") {
    return part.contentParts.reduce(
      (sum, item) => sum + getContentPartTextChars(item),
      0,
    );
  }
  return 0;
...
function getContentPartDataBytes(part: SerializedContentPart): number {
  ...
  if (part.type === "toolResult") {
    return part.contentParts.reduce(
      (sum, item) => sum + getContentPartDataBytes(item),
      0,
    );
  }
  return 0;
...
function flattenContentParts(
  parts: readonly SerializedContentPart[],
): SerializedContentPart[] {
